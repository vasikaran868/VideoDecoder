...
import com.example.videcoder.UtilsKt;
...
//        TestWrapper.runTest(this);
    ...
    public void testExtractDecodeEditEncodeMuxQVGA(File file, Context context, Uri uri, Surface su) throws Throwable {
        setSize(540, 800);
        setSource(R.raw.content_video);
        setCopyVideo();
        TestWrapper.runTest(this, file, context, uri, su);
    }
...
//        TestWrapper.runTest(this);
    ...
    private static class TestWrapper implements Runnable {
        ...
        private Uri mUri;
        private Surface surfa;
        private TestWrapper(ExtractDecodeEditEncodeMuxTest test, Context context, Uri uri, Surface surf) {
            mTest = test;
            mContext = context;
            mUri = uri;
            surfa = surf;
        ...
        public void run() {
            try {
//                mTest.extractDecodeEditEncodeMux(mContext, mUri, surfa);
            } catch (Throwable th) {
                mThrowable = th;
            }
        }
         ...
         * Entry point.
         */
        public static void runTest(ExtractDecodeEditEncodeMuxTest test, File file, Context context, Uri uri, Surface sur) throws Throwable {
            test.setOutputFile(file);
            TestWrapper wrapper = new TestWrapper(test, context, uri, sur);
...
//        }
//        sb.append(".mp4");
        mOutputFile = file.getAbsolutePath().toString();
    }
    /**
...
//            outputSurface.changeFragmentShader(FRAGMENT_SHADER);
     ...
     */
    private MediaExtractor createExtractor(Context context, Uri uri) throws IOException {
        ...
        while ((mCopyVideo && !videoEncoderDone) || (mCopyAudio && !audioEncoderDone)) {
            ...
            while (mCopyVideo && !videoExtractorDone
                ...
                if (videoExtractorDone) {
                    int mEosBufferIndex = videoDecoder.dequeueInputBuffer(TIMEOUT_USEC);
            ...
            while (mCopyAudio && !audioExtractorDone
                ...
                if (audioExtractorDone) {
                    int mEosBufferIndex = audioDecoder.dequeueInputBuffer(TIMEOUT_USEC);
...
//            assertEquals("no frame should be pending", -1, pendingAudioDecoderOutputBufferIndex);
        ...
        Log.v("Debug Tag", "reached here too");
